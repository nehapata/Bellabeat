...
sum(duplicated(Weight)) # checking for duplicates
# no duplicates found. 

Weight_Cleaned <- Weight %>% 
  select(-Fat) %>% # deselecting column 
  separate(Date, c("Dates", "Clocktime", "Am/Pm"), sep = " ") %>% # separating columns
  mutate(Dates = as.Date(Dates, format = "%m/%d/%Y")) %>% # correcting date format
  unite("Time", c("Clocktime", "Am/Pm"), sep = " ") # again uniting time with am/pm
